...
export function Header() {
  ...
  return (
    <header className="hidden md:block sticky top-0 z-40 bg-white">
      {/* ── Top Bar ── */}
      <div className="topbar py-1.5 px-4">
        ...
        <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-between gap-6">
          ...
          <Link href="/" className="flex-shrink-0 group">
            <div className="text-center select-none">
              <h1 className="text-3xl font-black tracking-tight text-text leading-none">
                SHEIN{" "}
          ...
          <div className="flex items-center gap-5">
            <Link href="/account" className="flex flex-col items-center text-text-light hover:text-primary transition-colors" id="header-account">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              <span className="text-xs mt-0.5">Compte</span>
            </Link>
            <Link href="/favorites" className="flex flex-col items-center text-text-light hover:text-primary transition-colors relative" id="header-favorites">
            ...
            <Link href="/cart" className="flex flex-col items-center text-text-light hover:text-primary transition-colors relative" id="header-cart">
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  <Link
                    href={link.href}
                    className={`block px-3.5 py-3 text-[13px] font-semibold transition-colors hover:text-primary ${
                      active
                        ? "text-primary border-b-[2.5px] border-primary"
                        : "text-text"
                    } ${link.label === "PROMOTIONS" ? "!text-[#ff4a79] !font-bold" : ""}`}
                  >
                    {link.label}
                  </Link>
