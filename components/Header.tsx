  ...
  return (
    <>
      <header
        ...
        className={`hidden md:block fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${active
          ? "bg-white/95 backdrop-blur-md shadow-sm"
          : "bg-transparent"
          }`}
      >
        {/* == Top Bar == */}
        <div className={`py-1.5 px-4 transition-colors duration-300 ${active ? "border-b border-border/30" : ""}`}>
          <div className="max-w-[1536px] mx-auto flex items-center justify-between text-[11px]">
            <div className={`flex items-center gap-5 transition-colors duration-300 font-semibold ${active ? "text-text-light" : "text-white/80"}`}>
            ...
            <div className={`flex items-center gap-3 transition-colors duration-300 font-semibold ${active ? "text-text-light" : "text-white/80"}`}>
              <span className="mr-1">Suivez-nous</span>
              <a href="#" className={`${active ? "hover:text-primary" : "hover:text-white"} transition-colors`} aria-label="Instagram">
              ...
              <a href="#" className={`${active ? "hover:text-primary" : "hover:text-white"} transition-colors`} aria-label="Facebook">
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
              </a>
              <a href="#" className={`${active ? "hover:text-primary" : "hover:text-white"} transition-colors`} aria-label="TikTok">
        ...
        <div className={`transition-colors duration-300 ${active ? "border-b border-border" : ""}`}>
          <div className="max-w-[1536px] mx-auto px-4 py-3 grid grid-cols-3 items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <span className={`text-sm font-medium ${active ? "text-gray-700" : "text-white/90"}`}>Menu</span>
              </button>
              ...
              >
                ...
                <span className={`text-sm font-medium ${active ? "text-gray-700" : "text-white/90"}`}>Rechercher</span>
              </button>
            </div>
              ...
              <Link href="/" className="flex-shrink-0">
                ...
                <img
                  ...
                  className={`h-12 w-auto transition-all duration-300 ${
                    active ? "" : "brightness-0 invert drop-shadow-md"
                  }`}
                />
              </Link>
            </div>
      ...
      {searchOpen && (
        <div className="hidden md:flex fixed inset-0 z-50 flex-col items-center justify-start">
          {/* Blurred backdrop */}
          <div
          ...
          {/* Content */}
          <div className="relative z-10 w-full max-w-2xl mx-auto px-6 pt-24 animate-fade-in-up">
            ...
            <form onSubmit={handleSearch} className="relative group">
              <div className="flex items-center bg-white rounded-2xl shadow-lg px-5 py-1">
                <input
                  ...
                  className="flex-1 py-3 text-base text-gray-800 placeholder-gray-400 bg-transparent appearance-none"
            ...
            {/* Popular searches */}
            <div className="mt-8">
              <p className="text-white/40 text-xs font-medium uppercase tracking-widest mb-3">Recherches populaires</p>
              <div className="flex flex-wrap gap-2">
                {["Robes", "Sacs", "Chaussures", "T-shirts", "Accessoires", "Nouveautés", "Ensemble"].map((term) => (
                  <button
                    key={term}
                    onClick={() => {
                      setSearchQuery(term);
                    ...
                    className="px-4 py-2 rounded-full bg-white/10 text-white/70 text-sm font-medium hover:bg-white/20 hover:text-white transition-all border border-white/10 hover:border-white/20"
                  >
                    {term}
                  </button>
                ))}
